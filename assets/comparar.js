  ...
  function render(){
    ...
    const priceRow = `
      <tr>
        ...
        ${items.map(p => { const pi = productPriceInfo(p); return `<td class="compare-price">${pi.isRange ? 'Desde ' : ''}${money(pi.price)}<span>${typeof priceSuffix === "function" ? priceSuffix() : "+ IVA"}</span></td>`; }).join('')}
      </tr>`;
